...
@st.cache()
def prediction(s_length, s_width, p_length, p_width, model):
  predicted_value = model.predict([[s_length, s_width, p_length, p_width]])
...
classifier = st.sidebar.selectbox('Classifier', ("Random Forest Classifier", "Support Vector Machines", "Logististic Regression"))
...
if st.sidebar.button('Predict'):
  if classifier == "Random Forest Classifier":
    species = prediction(s_length, s_width, p_length, p_width, rf_clf)
    score = rf_clf.score(X_train, y_train)
  elif classifier == "Support Vector Machines":
    species = prediction(s_length, s_width, p_length, p_width, svc_model)
    score = svc_model.score(X_train, y_train)
  else:
    species = prediction(s_length, s_width, p_length, p_width, log_reg)
